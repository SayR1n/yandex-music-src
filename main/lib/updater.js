...
class Updater {
    ...
    updateApplier(updateResult) {
        ...
        if (isVersionDeprecated() || !config_js_1.config.app.enableUpdateByProbability) {
            this.downloadUpdate(cancellationToken, updateInfo.version);
            return;
        }
        if ('updateProbability' in updateInfo && config_js_1.config.app.enableUpdateByProbability) {
